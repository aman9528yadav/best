      ...
      <div className="w-full max-w-[412px] flex flex-col flex-1">
        ...
        <main className="flex-1 space-y-4 pb-12">
           {isLoading ? (
             ...
             <>
                <p className="text-muted-foreground">Unlock powerful features by upgrading to Premium. Compare the plans below to see what you get.</p>
                ...
                <Card className="mt-8">
                    <CardHeader className="bg-muted/50 rounded-t-lg">
                        <div className="grid grid-cols-3 items-center font-semibold">
                            ...
                            <span className="text-center text-primary flex items-center justify-center gap-2"><Gem className="h-4 w-4"/>Premium</span>
                        </div>
                    </CardHeader>
                    <CardContent className="divide-y">
                        {features.map(item => (
                            <div key={item.id} className="grid grid-cols-3 items-center p-3">
                                <span className="font-medium text-sm">{item.feature}</span>
                                <div className="justify-self-center">
                                    {item.member ? <Check className="h-5 w-5 text-green-500" /> : <X className="h-5 w-5 text-red-500" />}
                                </div>
                                 <div className="justify-self-center">
                                    {item.premium ? <Check className="h-5 w-5 text-green-500" /> : <X className="h-5 w-5 text-red-500" />}
                                </div>
                            </div>
                        ))}
                    </CardContent>
